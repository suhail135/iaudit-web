...
import { useGetAuditTemplates, useSearchAuditTemplates } from '../api/template';

// ----------------------------------------------------------------------

export function AuditTemplateListView() {
  ...
  const { posts, postsLoading } = useGetAuditTemplates();

  const { searchResults, searchLoading } = useSearchAuditTemplates(debouncedQuery);
          ...
          <Tab
            ...
            icon={
              ...
              >
                ...
                {tab === 'published' && posts.filter((post: any) => post.published).length}

                {tab === 'draft' && posts.filter((post: any) => !post.published).length}
              </Label>
            }
...
const applyFilter = ({ inputData, filters, sortBy }: ApplyFilterProps) => {
  ...
  if (publish === 'published') {
    inputData = inputData.filter((post) => post.published);
  }
